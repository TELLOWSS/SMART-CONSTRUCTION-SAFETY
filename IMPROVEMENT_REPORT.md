...
## 📌 추가 개선 기록 (2026년 3월 4일)

### 1) 보고서 항목 포함/불포함 확장
- ✅ 보고서 탭에 **유도원/감시자 인건비 내역 포함 토글** 추가
- ✅ 기존 **안전시설 인건비 / 재료비 포함 토글**과 함께 독립 동작
- ✅ 둘 다 제외 시 인쇄 버튼에서 안내 경고 처리

### 2) 사진 이동 후 일일관리 미노출 결함 수정
- ✅ 일일관리 탭이 기존 유도원 사진만 보던 구조를 확장
- ✅ **안전시설 사진도 같은 날짜 기준으로 별도 카드 섹션 표시**
- ✅ 사진 이동 후에도 보고서/일일관리 양쪽에서 동일하게 확인 가능
- ✅ 날짜 저장 로직을 로컬 기준으로 보정하여 UTC 오프셋 누락 가능성 축소

### 3) Vercel 사용량 최적화 점검
- ✅ `GeminiAssistant`, `UserGuide`를 `React.lazy` + `Suspense`로 지연 로딩 전환
- ✅ 초기 로드 번들 부담 완화(첫 화면 JS 전송량 감소 목적)
- ⚠️ 에디터 가상 워크스페이스 경로 제약으로 로컬 터미널 빌드 결과 수집은 실패
  - PowerShell 정책: `npm` 스크립트 차단
  - 현재 경로: `package.json` 미탐지 (`C:\Users\user`)
- ▶ 권장 검증: 프로젝트 루트에서 `npm run build` 후 산출 크기 확인 + Vercel Analytics 확인

### 4) 서명 길이/위치 조정 기능
- ✅ 서명 등록 후 **크기/회전/좌우/상하** 슬라이더 제공
- ✅ 조정값이 즉시 보고서(갑지 서명란) 렌더링에 반영
- ✅ 기본 서명 스타일을 랜덤값에서 표준값(정중앙, 1배율)으로 변경
